...
def generate_heuristic_suggestions(query: str, jurisdiction: Optional[str], context_lines: list) -> list:
    ...
    entity = quoted_terms[0] if quoted_terms else (" ".join(key_terms[:3]) if key_terms else "")
    ...
    if not use_sqlite:
        try:
            ...
            if lens in ["comprehensive", "audits"]:
                try:
                    q = "SELECT report_num, jurisdiction, category, summary, dollar_impact, year, source_url FROM findings WHERE 1=1"
                    params = []
                    if juris_clean:
                ...
                for r in rows:
                    ...
                    context_lines.append(f"[DB-{db_idx}] SAO AUDIT{yr_str} - Agency: {r_dict['jurisdiction']} | Report: {r_dict['report_num']} | Category: {r_dict['category']} | Impact: {impact} | Summary: {r_dict['summary']}")
            ...
            if lens in ["comprehensive", "council"]:
                ...
                for r in cur_pg.fetchall():
                    ...
                    context_lines.append(f"[DB-{db_idx}] COUNCIL ACTION - Jurisdiction: {r['jurisdiction']} | Committee: {r['committee']} | Action: {r['key_action']} | Vendor: {r['vendor']} | Impact: {impact} | Vote: {r['vote_outcome']}")
                ...
                if conn_sao:
                    try:
                        ...
                        for sim_score, r in audit_items[:5]:
                            ...
                            context_lines.append(f"[DB-{db_idx}] SAO AUDIT ({yr}) - Agency: {r['jurisdiction']} | Report: {r['report_num']} | Category: {r['category']} | Impact: {impact} | Summary: {r['summary']}")
...
async def original_chat_stream_flow(query: str, lens: str, api_key: str, request: Request, history: Optional[List[dict]] = None):
    ...
    async def event_generator():
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Await intent results to identify jurisdiction
            jurisdiction, keywords = await intent_task
            ...
            # Await embedding results for vector correlation search
            ...
            use_sqlite = False
            try:
                conn_pg = await asyncio.to_thread(get_pg_conn)
                conn_pg.close()
            except Exception:
                use_sqlite = True
                
            # 2. Query Postgres/SQLite in thread pool (concurrent with generator context)
            context_lines, citations, correlations = await asyncio.to_thread(
            ...
            if bill_match:
                ...
                try:
                    bill_details = await asyncio.to_thread(get_legislative_bill_status_tool, bill_id)
                except Exception as e:
            ...
            if grant_match:
                ...
                try:
                    grant_details = await asyncio.to_thread(get_grants_by_category_tool, category, jurisdiction)
                except Exception as e:
...
"""
            ...
            async def generate_suggestions_api(query_str: str, ctx_lines: list) -> list:
                ...
                try:
                    ...
                    s_prompt = f"""Based on the user's query and the database context records, generate exactly 3 relevant follow-up questions for civic policy exploration.
...
def enrich_correlations_with_years(correlations_list: list) -> list:
    """Enrich correlation citations with their respective audit years and verbatim audit trails dynamically."""
    import re
                ...
                if report_num:
                    ...
                    if conn_2024:
                        try:
                            ...
                            cur.execute("SELECT COUNT(*) FROM findings WHERE report_num = ?", (report_num,))
                            if cur.fetchone()[0] > 0:
                                year = 2024
                            cur.close()
                            conn_2024.close()
                    ...
                    # Fetch verbatim audit trail context
                    verbatim_text = None
                    meeting_type = None
                    ...
                    try:
                        conn = get_pg_conn()
                        cur = conn.cursor()
                        cur.execute("SELECT verbatim_text_context, meeting_type, verification_score, reviewer_status FROM findings WHERE report_num = %s", (report_num,))
                        row = cur.fetchone()
                        cur.close()
                        conn.close()
                        if row:
                        ...
                        for db_name in ["sao_audits.db", "sao_2024.db"]:
                            ...
                            if conn:
                                try:
                                    cur = conn.cursor()
                                    cur.execute("SELECT verbatim_text_context, meeting_type, verification_score, reviewer_status FROM findings WHERE report_num = ?", (report_num,))
                                    row = cur.fetchone()
                                    cur.close()
                                    conn.close()
                                    if row:
